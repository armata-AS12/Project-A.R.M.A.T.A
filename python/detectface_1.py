...
cursor.execute('''
SELECT id staffId, username name, faceData 
FROM users
WHERE id = %s
''', (token,))
...
def is_within_check_in_time():
    ...
    # First check-in window: 8:00 AM to 9:00 AM
        ...
        if staff_id == user['staffId']:
            ...
            if is_within_check_in_time():
                if last_check_in_time is None or last_check_in_time.date() != now.date():
                    ...
                    cv2.putText(frame, 'Already registered today please check-out in Q', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            else:
                # Display message if outside check-in window
                cv2.putText(frame, 'Check-in time is over', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
